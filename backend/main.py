...
# city-keyed stores:  models[city][horizon], selected_features[city][horizon]
models = {}
selected_features = {}
model_metadata = {}
aqi_data = None
...
def load_models_and_data():
    ...
    try:
        ...
        for city in SUPPORTED_CITIES:
            ...
            city_meta = model_metadata.get(city, {})

            for horizon in [24, 48, 72]:
                # load selected features
                feat_path = os.path.join(base_dir, "config",
                                         f"selected_features_{city}_{horizon}h.json")
                # fallback to old single-city file name
                if not os.path.exists(feat_path):
                    feat_path = os.path.join(base_dir, "config",
                                             f"selected_features_{horizon}h.json")
                if os.path.exists(feat_path):
                    with open(feat_path, 'r') as f:
                        selected_features[city][horizon] = json.load(f)
                ...
                if not os.path.exists(model_path):
                    model_path = os.path.join(base_dir, "models",
                                              f"model_{horizon}h.pkl")
                if os.path.exists(model_path):
                    try:
                        model_obj = joblib.load(model_path)
                        models[city][horizon] = {'model': model_obj,
                                                 'type': 'ExtraTrees',
                                                 'format': 'pkl'}
                        total_models += 1
                        logger.info(f"Loaded model {city} {horizon}h")
                    except Exception as e:
                        logger.error(f"Error loading {city} {horizon}h: {e}")
                else:
        ...
        if os.path.exists(aqi_data_path):
            ...
            aqi_data['timestamp'] = pd.to_datetime(
                aqi_data['timestamp'], format='mixed', errors='coerce')
            logger.info(f"Loaded AQI data: {len(aqi_data)} records")

        data_loaded = total_models > 0 and aqi_data is not None
        logger.info(f"Data loading complete. Models loaded: {total_models}")
...
def get_default_value(feature_name):
    ...
    if 'aqi' in feature_lower:
        ...
        return 5.0
    elif any(p in feature_lower for p in ['pm25','pm10','o3','no2','so2','co']):
        return 10.0
    else:
        return 0.0

def load_horizon_data(horizon, city="karachi"):
    try:
        base_dir = os.path.dirname(os.path.abspath(__file__))
        # try city-specific file first, fall back to old name
        for fname in [f"horizon_{city}_{horizon}h_data.csv",
                      f"horizon_{horizon}h_data.csv"]:
            data_path = os.path.join(base_dir, "data", fname)
            if os.path.exists(data_path):
                df = pd.read_csv(data_path)
                return df.tail(72)
        logger.warning(f"Horizon data file not found for {city} {horizon}h")
        return None
    except Exception as e:
        logger.error(f"Error loading horizon data for {city} {horizon}h: {e}")
        return None

def get_features_for_prediction(horizon, city="karachi"):
    city = city.lower()
    city_feats = selected_features.get(city, {})
    if horizon not in city_feats:
        # fallback to karachi features
        city_feats = selected_features.get("karachi", {})
        if horizon not in city_feats:
            raise Exception(f"No selected features for {city} {horizon}h")

    features_needed = city_feats[horizon]
    horizon_df = load_horizon_data(horizon, city)

    if horizon_df is None or horizon_df.empty:
        return {f: get_default_value(f) for f in features_needed}
...
def make_prediction(horizon, feature_vector, city="karachi"):
    city = city.lower()
    city_models = models.get(city, {})
    if horizon not in city_models:
        # fallback to karachi model
        city_models = models.get("karachi", {})
        if horizon not in city_models:
            raise Exception(f"No model for {city} {horizon}h")

    model_info   = city_models[horizon]
    model_obj    = model_info['model']
    city_feats   = selected_features.get(city, selected_features.get("karachi", {}))
    features_needed = city_feats.get(horizon, [])

    ordered = [feature_vector.get(f, get_default_value(f)) for f in features_needed]
    pred_df = pd.DataFrame([ordered], columns=features_needed)
    prediction = model_obj.predict(pred_df)[0]
...
async def get_forecast(
    request: Request,
    location: Optional[str] = Query(default="Karachi", description="City name"),
    ...
    city = location.lower() if location else "karachi"

    try:
        forecasts = []
        horizons_to_run = [horizon] if horizon else sorted(
            models.get(city, models.get("karachi", {})).keys()
        )

        for h in horizons_to_run:
            feature_vector = get_features_for_prediction(h, city)
            prediction = make_prediction(h, feature_vector, city)
...
async def get_historical_data(
    ...
    try:
        ...
        # Fallback: if no data in the requested window, use the latest available records
        if df_filtered.empty:
            logger.info(f"No data in date window for {location}, falling back to latest {days*24} records")
            df_filtered = df_location.sort_values('timestamp').tail(days * 24).copy()
